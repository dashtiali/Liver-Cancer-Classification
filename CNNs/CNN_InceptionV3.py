...
# %% [markdown]
# ### CNN Setup and Training (Inception V3)
...
def train_model(params):
    ...
    aux_num_features = model.AuxLogits.fc.in_features
    model.AuxLogits.fc = nn.Linear(aux_num_features, 1)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            early_stop_counter = 0
            torch.save(model.state_dict(), 'InceptionV3_best_model.pth')
        else:
...
def evaluate_model(model, test_loader):
    ...
    recall = recall_score(patient_labels_binary,
                          patient_preds_binary, pos_label=1)
    precision = precision_score(
        patient_labels_binary, patient_preds_binary, pos_label=1)
    f1 = f1_score(patient_labels_binary, patient_preds_binary, pos_label=1)
...
model.load_state_dict(torch.load('InceptionV3_best_model.pth'))
model = model.to(device)
