...
if __name__ == '__main__':
    ...
    with tqdm(labels) as datasets_tqdm:
        for dataset_name in datasets_tqdm:
            datasets_tqdm.set_description(f'Dataset: {dataset_name}')
            
            pds_path = f'persistent_diagrams\\{dataset_name}_pds_dict.npy'
            pds_dict = np.load(pds_path, allow_pickle=True)
            pds_dict = pds_dict.item()
            pds_dict = [[file_name, pds] for file_name, pds in pds_dict.items() if len(pds) != 0]
            ...
            with tqdm(feature_list, colour='green') as feature_list_tqdm:
                for feature_name in feature_list_tqdm:
                    ...
                    pool = mp.Pool()
                    results = pool.map(Compute_feature, args)
                    pool.close()

                    # Feature matrix for PH_0
                    feature_matrix_dim_0 = np.column_stack((np.array([get_patient_ID(result[0]) for result in results]),
                                                            np.array([result[0] for result in results]),
                                                            np.array([result[1][0] for result in results])))
                    
                    dim_0_headers = ['Patient_ID', 'File_name'] + get_feature_names(feature_name, len(results[0][1][0]), 'dim(0)')

                    # Feature matrix for PH_1
                    feature_matrix_dim_1 = np.column_stack((np.array([get_patient_ID(result[0]) for result in results]),
                                                            np.array([result[0] for result in results]),
                                                            np.array([result[1][1] for result in results])))
                    
                    dim_1_headers = ['Patient_ID', 'File_name'] + get_feature_names(feature_name, len(results[0][1][1]), 'dim(1)')
